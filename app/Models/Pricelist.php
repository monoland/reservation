...
class Pricelist extends Model
{
    ...
    public function items()
    {
        return $this->belongsToMany(Item::class)->withPivot(
            'name',
            'price'
        )->withTimestamps()->orderBy('item_pricelist.id');
    }
    ...
    public static function storeRecord($request, $parent)
    {
        ...
        try {
            ...
            $model->sprice = $request->sprice;
            $model->segment_id = $request->segment['value'];
            $model->item_id = $request->item['value'];
            
            $parent->pricelists()->save($model);
            ...
            return new PricelistResource($model);
        } catch (\Exception $e) {
            DB::rollBack();
